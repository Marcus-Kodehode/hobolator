...
import CheatModal from '../shared/CheatModal'; // ⬅️ HUSK å importere denne

export default function HUD() {
  const [isInventoryOpen, setIsInventoryOpen] = useState(false);
  const [isCharacterOpen, setIsCharacterOpen] = useState(false);
  const [isCheatOpen, setIsCheatOpen] = useState(false); // ⬅️ Ny state
  ...
  useEffect(() => {
    ...
    setStats({ health, stamina, hygiene, cash, scrap, dayCount, timeSegment });
  }, []);
  ...
  return (
    <>
      ...
      <UtilityPanel
        ...
        onCheat={() => setIsCheatOpen(true)} // 🔁 Bruker setIsCheatOpen
      />
      ...
      {isCheatOpen && <CheatModal onClose={() => setIsCheatOpen(false)} />} {/* 👈 Nytt! */}
    </>
  );
}
