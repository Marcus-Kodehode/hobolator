...
export default function TopStatusPanel({ timeSegment = 1, dayCount = 1, cash = 0, scrap = 0 }) {
  const timeLabels = ['Morning', 'Afternoon', 'Evening', 'Night'];
  const currentTime = timeLabels[timeSegment - 1] || '???';
  ...
  return (
    <div className="absolute flex items-center gap-6 px-6 py-2 text-sm text-white -translate-x-1/2 border border-orange-400 rounded shadow-lg top-4 left-1/2 bg-black/50">
      <div>
        <span className="font-bold text-orange-300">Day {dayCount}</span> — {currentTime}
      </div>
